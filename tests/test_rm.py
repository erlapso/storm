...
class TestYouRM(unittest.TestCase):
    ...
    def test_forward_no_results(self, mock_get):
        # Prepare mock response with no hits
        mock_response = MagicMock()
        ...
        # Check if usage was incremented despite no results
        self.assertEqual(rm.usage, 1)

    @patch('requests.get')
    def test_forward_with_custom_is_valid_source(self, mock_get):
        # Prepare mock response
        mock_response = MagicMock()
        mock_response.json.return_value = {
            "hits": [
        ...
        self.assertEqual(len(results), 2)
        self.assertEqual(results[0]['url'], "https://example.com")
        self.assertEqual(results[1]['url'], "https://another-example.com")
